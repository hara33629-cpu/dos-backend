    ...
    try:
        ...
        <body style="font-family: Arial; background-color: #f4f4f4; padding: 20px;">
            <div style="max-width: 600px; margin: auto; background: white; padding: 20px; border-radius: 10px;">
                ...
                <h2 style="color: {'red' if decision=='BLOCK' else 'orange'};">
                    🚨 DOS Detection Alert
                </h2>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tr><td><strong>Threat Score:</strong></td><td>{threat_score:.3f}</td></tr>
                    <tr><td><strong>Time:</strong></td><td>{time.strftime('%Y-%m-%d %H:%M:%S')}</td></tr>
                </table>

                <h3>📊 Features</h3>
                <ul>
                    <li>Request Count: {features['request_count']}</li>
                    <li>High Rate: {features['high_request_rate']}</li>
                    <li>Repeated Access: {features['repeated_access']}</li>
                    <li>Small Payload: {features['small_payload']}</li>
                    <li>Large Payload: {features['large_payload']}</li>
                    <li>Bot Detected: {features['unusual_user_agent']}</li>
                </ul>

                <hr>

                <p style="color: gray;">
                    Generated by DOS Mitigation System
                </p>
...
def get_blocked_requests():
    try:
        ...
        cursor.execute("""
            SELECT id, ip, timestamp, method, user_agent,
                   request_size, threat_score, decision
            FROM traffic_logs
